...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%close all;
plot_mean_individual_strain=0; %plots mean data with each strain in a subplot, different conditions are plotted in different colours, repeats are plotted as individual lines with different line specs
...
plot_gr_histogram=0;%Plotting histogram of gr per trace
plot_width_histogram=0; %Plotting histogram of width for all repeats
plot_gr_histogram=0; %Plotting histogram of gr for all repeats
plot_hetero_cum=1; % Plotting cummaltive activation with a subplot for each strain and condition. A line is a repeat.
plot_all_data_for_one_repeat=0; %Plotting MY, Mean MY, gr and cummulative activation for one repeat

% plot_mean_hetero_cum=1;% Plotting cummaltive activation with a subplot for each strain and condition. A line is a repeat.
            ...
            for cond_now=1:length(condition);
                ...
                if isempty(D);
                    ...
                    for repeat_do=1:length(D)
                        ...
                        if remove_fat_cells==1;
                            data_temp=all_data{rep_now,strain_now,cond_now};
                            data_wid_all=data_temp.('wid');
                            data_wid=data_temp.('wid')(1:adjust_to,:);
                            data_wid_m=nanmean(data_wid(:));
                            data_wid_s=nanstd(data_wid(:));

    
                            goodones=sum(data_wid_all>(data_wid_m+6*data_wid_s))==0;
                            fnames=fieldnames(data_temp); %Getting all fieldnames
                            for fn=1:length(fnames)
                                %Getting all field names
                                temp_field_data=data_temp.(fnames{fn});
                        ...
                        if remove_slow_gr_cells==1
                            % Removing all non devidind cells (cell which
                            % grow slower than 0.5
                            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                            data_temp=all_data{rep_now,strain_now,cond_now};
                            data_wid_all=data_temp.('elong_rate');
%                             data_wid=data_temp.('elong_rate')(1:adjust_to,:);
                            data_wid_m=nanmean(data_wid_all,1);
%                             data_wid_s=nanstd(data_wid(:));

    
                            goodones=data_wid_m>=0.5;
                            fnames=fieldnames(data_temp); %Getting all fieldnames
                            for fn=1:length(fnames)
                                %Getting all field names
                                temp_field_data=data_temp.(fnames{fn});
...
if plot_mean_all==1;
    ...
    %Setting output 
    all_mean_data=cell(length(strains),length(condition));
    ...
    for strain_now=1:length(strains)
        for cond_now=1:length(condition)
            ...
            all_mean_data{strain_now,cond_now}=temp_data;
        end
    sgtitle(strrep(plot_what,'_',' '));
...
%                 end
                ...
                %if strain_now==1
                    m=nanmean(data_part(:));
                    s=nanmean(data_part(:));
                    thresh=m+sig_activation_thresh*s;
                %end
...
% Turn on dynamics cummulative plot
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if plot_all_data_for_one_repeat==1;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Description
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Plotting MY, Mean MY, gr and cummulative activation for one repeat
    % each row is is one repeat
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for strain_now=1:length(strains)
        for cond_now=1:length(condition)
            ...
            ind_cond=ind_cond+1;
            for rep_now=1:length(repeat_name)
                if mod(ii,16)==1&&~isempty(all_data{rep_now,strain_now,cond_now})
                   figure('units','normalized','outerposition',[0 0 1 1]);
                    ii=1;
                end
                ...
                %Plotting MY singel cell traces
                subplot(4,4,ii);
                plot(all_data{rep_now,strain_now,cond_now}.(plot_what)-200);
                hold on;
                plot(all_mean_data{strain_now,cond_now}(:,rep_now)-200,'r','Linewidth',2);
                hold on;
%                 %wide cells
%                 wid_now=all_data{rep_now,strain_now,cond_now}.('wid');
%                 wid_m=nanmean(wid_now(:));
%                 wid_s=nanstd(wid_now(:));
%                 goodones=sum(wid_now>(wid_m+wid_s*6))~=0;
%                 plot(all_data{rep_now,strain_now,cond_now}.(plot_what)(:,goodones)-200,'b', 'Linewidth',1);



                xlabel('frames');
                ylabel(plot_what);
                title([plot_what, all_data_names{rep_now,strain_now,cond_now}]);
                xline(adjust_to,'--k');
                
                data_all=all_data{rep_now,strain_now,cond_now}.(plot_what)-200;
                data_part=data_all(1:adjust_to,:);
                %data_part=data_all(1:switch_frame(day_now),:);
                %if strain_now==1
                    m=nanmean(data_part(:));
                ...
                %Plotting mean MY 
                subplot(4,4,ii+1);
                plot(all_mean_data{strain_now,cond_now}-200,'b');
                hold on;
                plot(all_mean_data{strain_now,cond_now}(:,rep_now)-200,'r');
                xlabel('frames');
                ylabel(plot_what);
                title([plot_what, all_data_names{rep_now,strain_now,cond_now}]);
                ...
                %Plotting MY singel cell traces
                subplot(4,4,ii+2);
                e_data=all_data{rep_now,strain_now,cond_now}.('elong_rate');
                e_data(e_data<0)=nan;
                plot(e_data);
                xlabel('frames');
                ylabel('Elong Rate');
                title(['Elong Rate', all_data_names{rep_now,strain_now,cond_now}]);
                ...
                %Plotting cum
                subplot(4,4,ii+3);
                plot(all_cum_data{strain_now,cond_now},'b');
                hold on;
                plot(all_cum_data{strain_now,cond_now}(:,rep_now),'r');
                xlabel('frames');
                ylabel('Fraction');
                title(['ON', all_data_names{rep_now,strain_now,cond_now}]);
                a=axis;
                axis([0,a(2),0,1]);
                ii=ii+4;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if plot_width_histogram==1;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Description
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Plotting histogram of width for all repeats
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %Setting index variables
    max_cond=length(strains)*length(condition);
    max_done_repeats_here=max2(sum(~cellfun(@isempty, all_data)));
    ...
    for strain_now=1:length(strains)
        for cond_now=1:length(condition)
            ...
            for rep_now=1:max_rep_now
                ...
                %Getting data
                data_in=all_data{rep_now,strain_now,cond_now};
                data_now=data_in.('wid');

                %Plotting Data
                subplot(max_done_repeats_here,max_cond,ind);
                histogram(data_now,0:0.25:max2(data_now),'normalization','probability');
%                 a=axis;
                %axis([0,a(2),0,1]);
...
%                 %Adding text with number of cells
%                 goodones=~isnan(data_now(last_frame,:));
                 text(0.1,0.8,['w: ',num2str(nanmean(data_now(:)))],'Unit','normalize');
                 xline(nanmean(data_now(:)),'r');
                 xline(nanmean(data_now(:))+3*nanstd(data_now(:)),'r');
...
% Plotting width histogram
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if plot_gr_histogram==1;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Description
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Plotting histogram of growth rate for all repeats
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %Setting index variables
    max_cond=length(strains)*length(condition);
    max_done_repeats_here=max2(sum(~cellfun(@isempty, all_data)));
    ...
    for strain_now=1:length(strains)
        ...
        for cond_now=1:length(condition)
            ...
            for rep_now=1:max_rep_now
                ...
                %Getting data
                data_in=all_data{rep_now,strain_now,cond_now};
                data_now=data_in.('elong_rate');
                data_now_gr_m=nanmean(data_now,1);

                %Plotting Data
                %subplot(max_done_repeats_here,max_cond,ind);
                subplot(4,4,ii);
                histogram(data_now_gr_m,0:0.1:max2(data_now_gr_m),'normalization','probability');
%                 a=axis;
                %axis([0,a(2),0,1]);
...
%                 %Adding text with number of cells
%                 goodones=~isnan(data_now(last_frame,:));
                 text(0.1,0.8,['gr: ',num2str(nanmean(data_now(:)))],'Unit','normalize');
                 xline(nanmean(data_now_gr_m(:)),'--r');
                 xline(nanmean(data_now_gr_m(:))+3*nanstd(data_now_gr_m(:)),'r');
                xline(nanmean(data_now_gr_m(:))+nanstd(data_now_gr_m(:)),'r');
                %Setting labels
                if mod(ind,max_cond)==1
                    ylabel('Fraction');
                end
